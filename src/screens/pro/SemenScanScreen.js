...
export default function SemenScanScreen({ navigation }) {
  const { isScanning, startScanner, stopScanner, mockCapture, capturedUri } =
    useCameraScanner();
  ...
  return (
    ...
    >
      <SectionHeader
        title="Semen Scan"
        subtitle="Image capture with AI analysis overlay"
      />

      <Card elevated>
        <View className="relative">
          <ScannerFrame
            label={isScanning ? "AI overlay active" : "Ready for sample image"}
          />
          {isScanning ? (
            <View className="absolute bottom-4 left-4 right-4 rounded-xl bg-zmen-primary/90 px-3 py-2">
              <Text className="text-xs font-semibold text-white">
                Analyzing edges and texture markers...
              </Text>
            </View>
          ) : null}
        </View>

        <Text className="mt-3 text-xs text-zmen-muted">
          Captured: {capturedUri || "No capture yet"}
        </Text>
